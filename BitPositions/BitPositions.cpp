...
// To run program:
// change to the directory files are stored in
// compile with g++ BitPositions.cpp -o run.exe
// run with ./run.exe text.txt
...
{
    if(((num >> --p1) & 1) == ((num >> --p2) & 1)) //subtracting 1 from each position because if p1 = 1 then no need to shift. It is the first element.
    {
        cout << "true" << endl;
    }
    else
